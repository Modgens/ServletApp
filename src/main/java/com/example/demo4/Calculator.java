...
public class Calculator {
    ...
    public static void main(String[] args) {
        ...
        Calculator calculator = new Calculator(start, end, step); // Створення об'єкта класу Calculator

        // Виклик методу fillX та виведення результату
        List<Double> listX = calculator.fillX();
        ...
        int maxIndex = calculator.getMax(listY);
        int minIndex = calculator.getMin(listY);
        double sum = calculator.calculateSum(listY);
        double average = calculator.calculateAverage(listY);
    ...
    public List<Double> fillX() {
        int size = getSize(start, end, step);
        return IntStream.range(0, size)
                .mapToDouble(i -> BigDecimal.valueOf(start).add(BigDecimal.valueOf(i).multiply(BigDecimal.valueOf(step))).doubleValue())
                .boxed()
                .collect(Collectors.toList());
    }

    public double[] fillY() {
        List<Double> listX = fillX();
        int size = listX.size();
